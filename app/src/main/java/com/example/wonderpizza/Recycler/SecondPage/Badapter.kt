...
class Badapter() :RecyclerView.Adapter<Bholder>() {
    ...
    private var ClickListener: OnCalClick?=null


    constructor(responseDTO: ResponseDTO,ClickListener: OnCalClick? ) : this() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Bholder {
        return  Bholder(LayoutInflater.from(parent.context).inflate(R.layout.item_crust,parent,false),ClickListener!!)
    }

    override fun onBindViewHolder(holder: Bholder, pos: Int) {
        var responseDTO: Crust = responseDTO!!.crusts[pos]
        holder.setData(responseDTO,pos,ClickListener!!)
    }
